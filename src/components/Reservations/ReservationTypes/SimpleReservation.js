...
import { FaPlusCircle, FaEraser } from 'react-icons/fa';
...
const SimpleReservation = ({
  ...
  useEffect(() => {
    getCorporateData();
  }, [userData, userCompany, companyName, vatNumber]); // eslint-disable-line
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (totalDays > workflow?.administration?.maxDays && workflow?.administration?.maxDays > 0) {
        toast.error(
          `Il Flusso selezionato permette Prenotazioni di massimo ${workflow?.administration?.maxDays
  ...
  return (
    <>
            ...
            <fieldset disabled={form.formState.isSubmitting}>
              ...
              <div className="flex flex-wrap gap-x-4">
                <div className="flex pt-2">
                  <div
                    className="flex-none w-72 pr-2"
                    onClick={() => {
                      if (!customDriver) setCustomDriverConfirm(true);
                    }}
                  >
                    <FormLabel>
                      Conducente{' '}
                ...
                <div className="flex pt-2">
                  <div
                    className="flex-none w-72 pr-2"
                    onClick={() => {
                      if (!customCustomer && mode !== 'corporate') setCustomClientConfirm(true);
                    }}
                  >
                    <FormLabel>
                      Cliente{' '}
                      {customCustomer === false && mode !== 'corporate' && (
                        <FaEraser
                          className="inline cursor-pointer"
                          title="Inserisci altro cliente"
                        />
                      )}
                    </FormLabel>
                    <TextInternal
                      ...
                      <tbody className="text-sm divide-y divide-gray-200">
                        {vehicles.map((vehicle, index) => (
                          ...
                          >
                            ...
                            <td className="first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                              <p className="text-left font-semibold text-gray-600">
                                ...
                                {vehicle.version?.group?.mnemonic !== undefined
                                  ? `${getVehicleGroup(vehicle).group?.mnemonic} - ${getVehicleGroup(vehicle).group?.description
                                  }`
                                  : 'Nessun Gruppo'}
                              </p>
                            </td>
                            ...
                            <td className="first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                              <p className="text-left font-semibold text-gray-600">
                                {storedDiscountAmount || storedDiscountPercentage ? storedDiscountAmount
                                  ? convertPrice(storedDiscountAmount) : `${storedDiscountPercentage}%` : 'N/A'}
                              </p>
                            </td>
                            <td className="first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                              <p className="text-left font-semibold text-gray-600">
                                {calculateFinalPrice(getVehicleGroup(vehicle).group) !== undefined ?
                                  convertPrice(calculateFinalPrice(getVehicleGroup(vehicle).group)) : 'N/A'}
                              </p>
                            </td>
                          </tr>
                        ))}
      ...
      {showDriverModal ? (
        <UsersModal inputType={'driver'} closeModal={closeDriverModal} returnUser={returnDriver} />
      ) : null}
      {showCustomerModal ? (
        <UsersModal
          inputType={'customer'}
