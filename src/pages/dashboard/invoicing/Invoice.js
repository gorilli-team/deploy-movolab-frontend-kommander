...
import { convertPrice, calculateScontoInverso } from '../../../utils/Prices';
...
import InvoiceMovoRecap from './InvoiceMovoRecap';
...
const Invoice = () => {
  ...
  const [paymentToReverse, setPaymentToReverse] = useState(undefined);
  ...
  useEffect(() => {
    ...
    getPaymentForCreditNote(invoice);
  }, [invoice]);
  ...
  const getPaymentForCreditNote = async (invoice) => {
    try {
      if (invoice.creditNoteFor !== undefined) {
        const paymentId = invoice.creditNoteFor?.paymentData?.paymentId;
        const payment = await http({
          url: `/payments/payments/${paymentId}`,
        });
  ...
  const calculateInvoiceAmounts = (invoice) => {
    ...
    const totalDiscount = invoice.lines.reduce(
      (acc, line) => acc + calculateScontoInverso(line),
      0,
    );
    const totalImponibile = invoice.price.priceNoVat - totalDiscount;
    const totalVat = Number((invoice.price.finalPrice - totalImponibile).toFixed(2));

    const amounts = {
      initialPrice: invoice.price.priceNoVat,
      totalDiscount,
      totalImponibile,
      totalVat,
      finalPrice: invoice.price.finalPrice,
    };
  ...
  const stornaPagamento = async (paymentToReverse) => {
    try {
      if (paymentToReverse?.paymentMethod === 'banco') {
        await http({
          url: `/payments/payments/refund`,
          method: 'POST',
          form: {
        ...
        fetchInvoice();
      } else if (paymentToReverse.paymentMethod === 'stripe') {
        if (!paymentToReverse?.stripePaymentIntent) {
          return toast.error('Storno non possibile');
        } else {
          await http({
            url: `/payments/stripe-connect/refund`,
            method: 'POST',
            form: {
              stripePaymentIntent: paymentToReverse?.stripePaymentIntent,
              invoicingType: invoice?.invoicingType,
            },
          });
        }
        toast.success('Pagamento stornato');
        ...
        toast.error('Metodo di pagamento non supporta storno');
      }
    } catch (error) {
      console.error('error', error);
      if (error.error === 'charge_already_refunded') {
        toast.error('Errore. Pagamento già Rimborsato.');
      } else {
        toast.error('Non è stato possibile stornare il pagamento');
      }
    }
  };
  ...
  return (
    <Page canAccess={[CLIENT_ROLE_ADMIN, CLIENT_ROLE_OPERATOR]} bodyClassName="pb-4 px-2">
      <CardsHeader
        title="Fattura"
        ...
        buttons={[
          ...
          },
          {
            ...
            hiddenIf: invoice.invoicingType !== 'customer',
          },
          {
            btnStyle: 'slate',
            children: 'Emetti Nota di Credito',
            onClick: stornaFattura,
            hiddenIf: isCreditNoteAvailable || invoice.invoicingType === 'movolab',
          },
          {
            btnStyle: 'blue',
            children: 'Scarica XML',
            onClick: generateFatturaElettronica,
            hiddenIf: invoice.invoicingType === 'movolab',
          },
          {
            btnStyle: 'blue',
            children: 'Stampa',
      ...
      <WhiteBox className="mt-0 p-6 print:mx-0 print:shadow-none print:p-0">
        <div className="flex justify-between flex-wrap">
          ...
          {invoice.invoicingType === 'customer' && (
            <div className="lg:min-w-[18rem] text-right">
                  ...
                  <tbody>
                    {invoice.lines.map((line, index) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-2">{convertPrice(calculateScontoInverso(line))}</td>
                        <td className="px-4 py-2">
                          {convertPrice(line.initialPrice - calculateScontoInverso(line))}
                        </td>
              ...
              <div className="flex">
                ...
                <div className="bg-white rounded-2xl overflow-auto mx-0 border border-gray-300 font-medium text-gray-900 mt-5 w-full md:w-1/2 divide-y">
                  <div className="flex justify-between px-4 py-2">
                    <span>Prezzo</span>
                    <span>{convertPrice(invoiceAmounts.initialPrice)}</span>
                  </div>
                  <div className="flex justify-between px-4 py-2">
                    <span>Sconto</span>
                    <span>{convertPrice(invoiceAmounts.totalDiscount)}</span>
                  </div>
                  <div className="flex justify-between px-4 py-2">
                    <span>Imponibile</span>
                    <span>{convertPrice(invoiceAmounts.totalImponibile)}</span>
                  </div>
                  {invoiceAmounts.totalVat > 0 && (
                    <div className="flex justify-between px-4 py-2">
                      <span>Iva</span>
                      <span>{convertPrice(invoiceAmounts.totalVat)}</span>
                    </div>
                  )}

                  <div className="flex justify-between px-4 py-2 bg-slate-100 font-semibold">
                    <span>Prezzo Finale</span>
                    <span>{convertPrice(invoiceAmounts.finalPrice)}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        {paymentToReverse !== undefined && (
          <div className="mt-3 bg-white rounded-2xl overflow-hidden mx-0 border border-gray-300">
            <div className="flex justify-between p-3">
              <div className="font-bold text-lg">
                {paymentToReverse?.paymentStatus !== 'refunded'
                  ? 'Pagamento da Stornare'
                  : 'Pagamento Stornato'}
              </div>
              {paymentToReverse?.paymentStatus !== 'refunded' ? (
                <Button onClick={() => stornaPagamento(paymentToReverse)} className="!py-1 !h-auto" btnStyle={'inFormStyle'}>
                  Storna Pagamento
                </Button>
              ) : (
                <ElementLabel bgColor="bg-yellow-600" className="uppercase">
