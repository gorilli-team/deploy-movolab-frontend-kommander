  ...
  const fetchVehicles = async (skip = 0, limit = 10) => {
    try {
      const response = await http({
        url: `/admin/vehicle?rentalLocation=${params.id}&skip=${skip}&limit=${limit}`,
      });
  ...
  const onSubmit = async (data) => {
    try {
      if (mode === 'create') {
        ...
        toast.success('Punto nolo aggiunto');
        history.goBack();
      } else if (mode === 'edit') {
        await http({
          method: 'PUT',
  ...
  return (
    <AdminPage canAccess={[MOVOLAB_ROLE_ADMIN]} hasBox={false}>
      ...
      <WhiteBox className="mt-0 !overflow-visible">
        ...
        <div
          className={`bg-slate-200 border-4 rounded-b-2xl border-white ${
            fieldToUpdate !== 'vehicles' && 'p-4'
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} id="rentLocationForm">
                  <fieldset disabled={form.formState.isSubmitting}>
                    <div className="max-w-sm">
                      <FormLabel>Nome Punto Nolo</FormLabel>
                      <TextField
                        form={form}
                        name="name"
                        type="string"
                        ...
                        validation={{
                          required: { value: true, message: 'Nome Punto Nolo' },
                        }}
                      />
                      <FormLabel>Indirizzo</FormLabel>
                      <TextField form={form} name="address" type="string" placeholder="Indirizzo" />
                      <FormLabel>Telefono</FormLabel>
                      <TextField form={form} name="phone" type="string" placeholder="Telefono" />
...
export default AdminRentalLocation;
