...
const Versions = () => {
  ...
  const setSelectedBrandAndReload = (brandId) => {
    ...
    history.push(`/admin/veicoli/versioni?marca=${brandId}`);
  };

  const setSelectedModelAndReload = (model) => {
    setSelectedModel(model);
    fetchVersions(selectedBrand, model, 0, 10);
    history.push(`/admin/veicoli/versioni?marca=${selectedBrand}&modello=${model}`);
  };
  ...
  const fetchVersions = async (brandId, modelId, skip = 0, limit = 10) => {
    try {
      ...
      if (brandId) {
        ...
        response = await http({
          url: `/vehicles/version?skip=${skip}&limit=${limit}`,
        });
      }
      ...
      <div className="flex justify-between items-center">
        <TableHeader
          ...
          buttons={[
            {
              function: () => {
                history.push('/admin/veicoli/versioni/crea');
              },
